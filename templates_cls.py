...
def ffhq256_autoenc_cls():
    ...
    conf.manipulate_mode = ManipulateMode.celebahq_all
    conf.manipulate_znormalize = True
    conf.latent_infer_path = f'/home/coolboy/wwh/diffusion-autoencoders-main/checkpoints/{ffhq256_autoenc().name}/latent.pkl'  # we train on Celeb dataset, not FFHQ
    conf.batch_size = 32
    conf.lr = 1e-3
    conf.total_samples = 300_000
    # use the pretraining trick instead of contiuning trick
    conf.pretrain = PretrainConfig(
        '130M',
        f'/home/coolboy/wwh/diffusion-autoencoders-main/checkpoints/{ffhq256_autoenc().name}/last.ckpt',
    )
